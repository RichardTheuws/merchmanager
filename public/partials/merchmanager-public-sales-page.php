...
// Get band and show data (ensure valid)
$band = $band_id ? get_post( $band_id ) : null;
$show = $show_id ? get_post( $show_id ) : null;
if ( ! $band || 'msp_band' !== get_post_type( $band_id ) ) {
    echo '<p>' . esc_html__( 'Band not found for this sales page.', 'merchmanager' ) . '</p>';
    return;
}
...
if ( $require_access_code ) {
    ...
    if ( isset( $_POST['msp_access_code'] ) ) {
        if ( sanitize_text_field( wp_unslash( $_POST['msp_access_code'] ) ) === $access_code ) {
...
<div class="msp-sales-page">
    <div class="msp-sales-page-header">
        ...
        <div class="msp-sales-page-info">
            ...
            <?php if ( $show_id && $show && 'msp_show' === get_post_type( $show_id ) ) : ?>
                <p class="msp-show-info">
                    <?php
                    // Format show date
                    $msp_options = get_option( 'msp_settings', array() );
                    $date_format = ( is_array( $msp_options ) && isset( $msp_options['date_format'] ) ) ? $msp_options['date_format'] : 'm/d/Y';
                    $time_format = ( is_array( $msp_options ) && isset( $msp_options['time_format'] ) ) ? $msp_options['time_format'] : 'g:i a';
    ...
    <?php if ( $require_access_code && ! $access_granted ) : ?>
        <div class="msp-access-code-form">
            <h2><?php esc_html_e( 'Access Code Required', 'merchmanager' ); ?></h2>
            <p><?php esc_html_e( 'Please enter the access code to view this sales page.', 'merchmanager' ); ?></p>
            <?php if ( $access_code_error ) : ?>
                <p class="msp-error"><?php esc_html_e( 'The access code you entered is incorrect. Please try again.', 'merchmanager' ); ?></p>
            <?php endif; ?>
            
            <form method="post" action="">
                <div class="msp-form-group">
                    <label for="msp_access_code"><?php esc_html_e( 'Access Code', 'merchmanager' ); ?></label>
                    <input type="text" name="msp_access_code" id="msp_access_code" required />
                </div>
                
                <div class="msp-form-group">
                    <button type="submit" class="msp-button"><?php esc_html_e( 'Submit', 'merchmanager' ); ?></button>
                </div>
            </form>
        </div>
    <?php else : ?>
        <div class="msp-sales-form">
            <h2><?php esc_html_e( 'Record Sale', 'merchmanager' ); ?></h2>
                ...
                <div class="msp-merchandise-list">
                    ...
                    if ( ! empty( $merchandise_ids ) && is_array( $merchandise_ids ) ) {
                        foreach ( $merchandise_ids as $merchandise_id ) {
                            ...
                            // Format price
                            $msp_options = get_option( 'msp_settings', array() );
                            $currency = ( is_array( $msp_options ) && isset( $msp_options['currency'] ) ) ? $msp_options['currency'] : 'USD';
                            ...
                            <div class="msp-merchandise-item" data-id="<?php echo esc_attr( $merchandise_id ); ?>" data-price="<?php echo esc_attr( $price ); ?>">
                                ...
                                <div class="msp-merchandise-quantity">
                                    <label for="msp_quantity_<?php echo esc_attr( $merchandise_id ); ?>"><?php esc_html_e( 'Quantity', 'merchmanager' ); ?></label>
                        ...
                        echo '<p>' . esc_html( __( 'No merchandise available.', 'merchmanager' ) ) . '</p>';
                    }
                    ?>
                </div>
                
                <div class="msp-sale-summary">
                    <h3><?php esc_html_e( 'Sale Summary', 'merchmanager' ); ?></h3>
                    
                    <div class="msp-sale-items">
                        <p class="msp-no-items"><?php esc_html_e( 'No items selected.', 'merchmanager' ); ?></p>
                        <ul class="msp-selected-items"></ul>
                    </div>
                    
                    <div class="msp-sale-total">
                        <p><?php esc_html_e( 'Total:', 'merchmanager' ); ?> <span class="msp-total-amount"><?php echo esc_html( $currency_symbol ); ?>0.00</span></p>
                    </div>
                    
                    <div class="msp-payment-type">
                        <label for="msp_payment_type"><?php esc_html_e( 'Payment Type', 'merchmanager' ); ?></label>
                    ...
                    <div class="msp-notes">
                        <label for="msp_notes"><?php esc_html_e( 'Notes', 'merchmanager' ); ?></label>
                        <textarea name="msp_notes" id="msp_notes" rows="3"></textarea>
                    </div>
                    
                    <div class="msp-submit">
                        <button type="submit" class="msp-button msp-record-sale" disabled><?php esc_html_e( 'Record Sale', 'merchmanager' ); ?></button>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div class="msp-sale-confirmation" style="display: none;">
            <h2><?php esc_html_e( 'Sale Recorded', 'merchmanager' ); ?></h2>
            <p><?php esc_html_e( 'The sale has been recorded successfully.', 'merchmanager' ); ?></p>
            <p><a href="" class="msp-button msp-new-sale"><?php esc_html_e( 'Record Another Sale', 'merchmanager' ); ?></a></p>
        </div>
    <?php endif; ?>
</div>