...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
    if ( isset( $_POST['msp_add_to_sale'] ) ) {
        ...
        if ( is_wp_error( $result ) ) {
            ...
            // Refresh the page to show updated sales session
            wp_redirect( add_query_arg( array( 'added' => $merchandise_id ), $_SERVER['REQUEST_URI'] ) );
            exit;
        }
    }
    
    if ( isset( $_POST['msp_record_sale'] ) ) {
    ...
    if ( isset( $_POST['msp_update_quantity'] ) ) {
    ...
    if ( isset( $_POST['msp_remove_item'] ) ) {
...
<div class="msp-sales-recording">
    <h1><?php _e( 'Manual Sales Recording', 'merchmanager' ); ?></h1>
    ...
    <div class="msp-sales-filters">
        <form method="get" action="">
            <label for="band_id"><?php _e( 'Filter by Band:', 'merchmanager' ); ?></label>
            ...
            <?php if ( ! empty( $shows ) ) : ?>
                <label for="show_id"><?php _e( 'Filter by Show:', 'merchmanager' ); ?></label>
                <select name="show_id" id="show_id" onchange="this.form.submit()">
                    <option value="0"><?php _e( 'All Shows', 'merchmanager' ); ?></option>
    ...
    <div class="msp-sales-content">
        <div class="msp-merchandise-list">
            <h2><?php _e( 'Available Merchandise', 'merchmanager' ); ?></h2>
                    ...
                    <div class="msp-merchandise-item">
                        ...
                        <form method="post" action="" class="msp-add-to-sale-form">
                            ...
                            <div class="msp-quantity-controls">
                                <label for="quantity_<?php echo esc_attr( $item->ID ); ?>"><?php _e( 'Quantity:', 'merchmanager' ); ?></label>
                                <input type="number" name="quantity" id="quantity_<?php echo esc_attr( $item->ID ); ?>" min="1" max="<?php echo esc_attr( $stock ); ?>" value="1">
                                <button type="submit" name="msp_add_to_sale" class="msp-button"><?php _e( 'Add to Sale', 'merchmanager' ); ?></button>
                            </div>
                        </form>
                    </div>
                <?php endforeach; ?>
            <?php else : ?>
                <p><?php _e( 'No merchandise available.', 'merchmanager' ); ?></p>
            <?php endif; ?>
        </div>
        
        <div class="msp-sales-session">
            <h2><?php _e( 'Current Sale', 'merchmanager' ); ?></h2>
                    ...
                    <table class="msp-sale-table">
                        <thead>
                            <tr>
                                ...
                                <th><?php _e( 'Subtotal', 'merchmanager' ); ?></th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form method="post" action="" style="display: inline;">
                                            ...
                                            <button type="submit" name="msp_update_quantity" class="msp-button-small"><?php _e( 'Update', 'merchmanager' ); ?></button>
                                        </form>
                                    </td>
                                    ...
                                    <td>
                                        <form method="post" action="" style="display: inline;">
                                            ...
                                            <input type="hidden" name="merchandise_id" value="<?php echo esc_attr( $item['merchandise_id'] ); ?>">
                                            <button type="submit" name="msp_remove_item" class="msp-button-small msp-button-remove"><?php _e( 'Remove', 'merchmanager' ); ?></button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="3" align="right"><strong><?php _e( 'Total:', 'merchmanager' ); ?></strong></td>
                ...
                <div class="msp-sale-actions">
                    <form method="post" action="">
                        ...
                        <div class="msp-sale-details">
                            <h3><?php _e( 'Sale Details', 'merchmanager' ); ?></h3>
                            
                            <div class="msp-form-group">
                                <label for="payment_type"><?php _e( 'Payment Type:', 'merchmanager' ); ?></label>
                            ...
                            <?php if ( ! empty( $shows ) ) : ?>
                                <div class="msp-form-group">
                                    <label for="show_id"><?php _e( 'Show:', 'merchmanager' ); ?></label>
                                    <select name="show_id" id="show_id">
                                        <option value="0"><?php _e( 'Not associated with a show', 'merchmanager' ); ?></option>
                                        <?php foreach ( $shows as $show ) : ?>
                                            <option value="<?php echo esc_attr( $show->ID ); ?>" <?php selected( $current_show_id, $show->ID ); ?>>
                                                <?php echo esc_html( $show->post_title ); ?>
                                            </option>
                            ...
                            <div class="msp-form-group">
                                <label for="notes"><?php _e( 'Notes:', 'merchmanager' ); ?></label>
                                <textarea name="notes" id="notes" rows="3" placeholder="<?php _e( 'Optional notes about this sale...', 'merchmanager' ); ?>"></textarea>
                            </div>
                        </div>
                        
                        <div class="msp-action-buttons">
                            <button type="submit" name="msp_record_sale" class="msp-button msp-button-primary">
                                <?php _e( 'Record Sale', 'merchmanager' ); ?>
                            </button>
                            
                            <button type="submit" name="msp_clear_sale" class="msp-button msp-button-secondary" onclick="return confirm('<?php _e( 'Are you sure you want to clear the current sale?', 'merchmanager' ); ?>')">
                                <?php _e( 'Clear Sale', 'merchmanager' ); ?>
                            </button>
                        </div>
                    </form>
                </div>
            <?php else : ?>
                <p class="msp-no-items"><?php _e( 'No items in current sale. Add items from the merchandise list.', 'merchmanager' ); ?></p>
            <?php endif; ?>
        </div>
    </div>
</div>